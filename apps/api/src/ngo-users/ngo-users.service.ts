...
export class NgoUsersService {
  ...
  async create(createNgoUserDto: {
    ...
    // Validate that the NGO exists and is verified
  ...
  async updateProfile(id: string, updateData: {
    ...
    const updatedUser = await this.ngoUserModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true }
  ...
  async updatePassword(id: string, hashedPassword: string): Promise<void> {
    const result = await this.ngoUserModel.findByIdAndUpdate(
      id,
